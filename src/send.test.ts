...
describe('WebexSender', () => {
  ...
  describe('send', () => {
    ...
    it('should clamp adaptive card version above 1.3 down to 1.3', async () => {
      mockFetch.mockResolvedValueOnce(createMockResponse(mockMessage));
      ...
      expect(body.attachments[0].content.version).toBe('1.3');
    });

    it('should default a missing or malformed adaptive card version to 1.3', async () => {
      mockFetch.mockResolvedValueOnce(createMockResponse(mockMessage));
      mockFetch.mockResolvedValueOnce(createMockResponse(mockMessage));

      const missingVersion: OpenClawOutboundMessage = {
        to: 'room-123',
        content: {
          text: 'Card message',
          card: {
      ...
      const malformedVersion: OpenClawOutboundMessage = {
        to: 'room-123',
        content: {
          text: 'Card message',
          card: {
      ...
      await sender.send(missingVersion);
      await sender.send(malformedVersion);

      const bodies = mockFetch.mock.calls.map(
        (call) => JSON.parse((call[1] as { body: string }).body)
      );
      expect(bodies[0].attachments[0].content.version).toBe('1.3');
      expect(bodies[1].attachments[0].content.version).toBe('1.3');
    });

    it('should derive fallback text from the card body when no text or markdown is set', async () => {
      mockFetch.mockResolvedValueOnce(createMockResponse(mockMessage));
      ...
      const call = mockFetch.mock.calls[0];
      const body = JSON.parse((call[1] as { body: string }).body);
      expect(body.text).toBe('Hello from card');
    });

    it('should fall back to a generic message when the card has no usable TextBlock', async () => {
      mockFetch.mockResolvedValueOnce(createMockResponse(mockMessage));
      ...
      const call = mockFetch.mock.calls[0];
      const body = JSON.parse((call[1] as { body: string }).body);
      expect(body.text).toBe('Interactive card');
    });

    it('should not override existing text with card fallback text', async () => {
      mockFetch.mockResolvedValueOnce(createMockResponse(mockMessage));
